...
public class InvertedIndex : IDisposable
{
    ...
    /// <summary>
    /// Instantiates new index with persistence set to disk, rebuilds the index automatically if needed.
    /// </summary>
    /// <param name="indexPath">Directory in which index files will be stored. The directory will be created if it doesn't exist.</param>
    ...
    /// <summary>
    /// Rebuilds the index meta information, such as version of the index.
    /// </summary>
    ...
    /// Indexes given document. If the same document is already indexed, it is updated.
    /// </summary>
    /// <param name="content">Text of the document - any string</param>
    /// <param name="id">ID of the document. Must be unique per document.</param>
    ...
    /// <summary>
    /// Synchronizes the index, optionally reports progress.
    /// Missing documents are indexed, existing documents are updated, all documents not present in the <see cref="sourceDocuments"/> are removed from the index.
    /// </summary>
    /// <param name="sourceDocuments">All documents which should be in the index. Any indexed documents not present in this list are removed from the index.</param>
    /// <param name="progress">Reports progress after every 10% of work.</param>
    ...
    /// <summary>
    /// Searches for matches in the index. The search can be configured, but by default acts as a fulltext search, any cases where <see cref="string.Contains(string)"/> would match are matched.
    /// </summary>
    /// <param name="substring">The needle to search for</param>
    /// <param name="caseSensitive">Whether the search should be case-sensitive</param>
    /// <param name="wholeWords">Whether to match whole words. Word boundaries are: whitespace characters, tabs, new lines</param>
    /// <param name="page">Which page of results should be returned, starts from 1</param>
    /// <param name="pageSize">How many results should be reported</param>
    /// <returns></returns>
